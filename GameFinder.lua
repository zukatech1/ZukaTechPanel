...
local titleBarHeight = IsOnMobile and 48 or 42
local searchBarHeight = IsOnMobile and 40 or 36
local sortButtonWidth = IsOnMobile and 100 or 90
...
local buttonTpWidth = IsOnMobile and 130 or 120
local buttonCpWidth = IsOnMobile and 100 or 90
local buttonSrvWidth = IsOnMobile and 120 or 110
local expHeaderHeight = IsOnMobile and 36 or 32
local expListHeight = IsOnMobile and 200 or 180
local szDesktop = UDim2.new(.5,0,.6,0)
local szMobile = UDim2.new(.94,0,.75,0)
...
SORT.Position = UDim2.new(1,-innerPadding,.5,0)
...
LST.Position = UDim2.new(0,innerPadding,0,innerPadding)
LST.Size = UDim2.new(1,-innerPadding*2,1,-innerPadding*2)
...
ROW_TXT.Size = UDim2.new(.58,-innerPadding,1,0)
...
EMP.Position = UDim2.new(.5,0,1,-innerPadding)
EMP.Visible = false
...
local function hideBody(h)
	minimized = h
	if h then
		tw(M,.2,nil,nil,{Size=UDim2.new(normSize.X.Scale,normSize.X.Offset,0,titleBarHeight)}):Play()
...
local function toast(ti,tx,dur)
	...
	task.delay(dur,function()
		tw(f,.18,nil,Enum.EasingDirection.In,{BackgroundTransparency=1,Size=UDim2.new(0,IsOnMobile and 360 or 320,0,0)}):Play()
...
local function closeRow(r)
	...
	r.expOpen=false
	tw(r.f,.16,nil,Enum.EasingDirection.In,{Size=UDim2.new(1,-4,0,rowHeight)}):Play()
	tw(r.exp,.16,nil,Enum.EasingDirection.In,{Size=UDim2.new(1,-innerPadding*2,0,0),BackgroundTransparency=1}):Play()
...
local function mk(pl,idx)
	...
	semp.Position = UDim2.new(.5,0,1,-innerPadding / 2)
	semp.ZIndex = 7
	semp.Visible = false
	...
	bsrv.MouseButton1Click:Connect(function()
		if it.expOpen then
			...
			it.exp.Visible=true
			it.expOpen=true
			tw(it.f,.16,nil,nil,{Size=UDim2.new(1,-4,0,rowHeight + expHeaderHeight + expListHeight + innerPadding * 2)}):Play()
			tw(it.exp,.16,nil,nil,{Size=UDim2.new(1,-innerPadding*2,0,expListHeight),BackgroundTransparency=.05}):Play()
	...
	task.delay(idx*0.012,function()
		...
		tw(f,.18,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,{Size=UDim2.new(1,-4,0,rowHeight),BackgroundTransparency=.1}):Play()
